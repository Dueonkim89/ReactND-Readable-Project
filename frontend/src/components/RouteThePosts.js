		...
		return (
				<div>
					...
					{postInfo !== null && posts.filter(post => post.id === postInfo).map( eachPost => (
						...
						{/* Filter comments by parentId to get relevant comments, Then map  DisplayComments component*/}
						...
						{comments.filter( x => x.parentId === postInfo).map( eachComment => (
							<div key={eachComment.id} className='commentDiv'>
								<Row className="commentContainer">
									<Col xs={1} md={1} className='iconDiv'>
										<a className="topArrow" onClick={ ()=> console.log('create method for comment arrow') }>	
											<img src={upArrowIcon} alt='Up Arrow Icon'/>
										</a> 
										<a className="downArrow" onClick={ ()=> console.log('create method for comment arrow') }>																
											<img src={downArrowIcon} alt='Down Arrow Icon'/>
										</a>
